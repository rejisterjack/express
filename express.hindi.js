...
class ExpressIsko {
  ...
  sabkeLiye(...args) {
    if (typeof args[0] === "string") {
      ...
      handlers.forEach((handler) => {
        this.beachMain.push({ path, handler })
      })
    } else {
      args.forEach((handler) => {
        this.beachMain.push({ path: null, handler })
      })
    }
  }
  ...
  suno(port, callback) {
    const server = http.createServer(async (req, res) => {
      ...
      try {
        ...
        await this.runBeachMain(enhancedReq, enhancedRes)
    ...
    return new Promise((resolve, reject) => {
      ...
      req.on("end", () => {
        try {
          ...
          res.status(400).bhejo("Invalid JSON")
          reject(err)
        }
      })
      req.on("error", reject)
  ...
  async runBeachMain(req, res) {
    for (const layer of this.beachMain) {
  ...
  async handleRequest(req, res) {
    ...
    if (!handlers) {
      return res.status(404).bhejo("Nahi Mila")
    }
...
function expressIsko() {
  return new ExpressIsko()
}

module.exports = expressIsko
